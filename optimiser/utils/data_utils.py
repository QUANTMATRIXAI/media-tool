...
def validate_beta_file(df):
    """Validate beta coefficients file structure (supports both standard and Kalman formats)."""
    try:
        if df.empty:
            return False, "Beta file is empty"
        
        # Import beta converter
        from .beta_converter import detect_beta_format
        
        # Detect format
        format_type = detect_beta_format(df)
        
        if format_type == 'kalman':
            # Kalman format - check for required columns
            if 'Product' not in df.columns:
                return False, "Kalman format requires 'Product' column"
            if 'Intercept' not in df.columns:
                return False, "Kalman format requires 'Intercept' column"
            # Has variables (any column that's not Product/metrics/Intercept)
            ...
            if len(variable_cols) == 0:
                return False, "Kalman format must have at least one variable column"
            return True, "Kalman modeling format detected - will be auto-converted"
        
        elif format_type == 'standard':
            # Standard format - original validation
            ...
            return False, "Unknown beta file format. Must be either standard (Product title, B0, Beta_*) or Kalman (Product, Intercept, variables)"
...
def get_channel_beta_mapping():
    ...
    return {
        ...
        "traffic(all sku's)": 'Beta_Daily_Impressions_LINK_CLICKS',  # Updated to match Kalman format
        "catalog campaign(all sku's)": None
    }


def get_channel_beta_mapping_with_fallback(beta_df):
    """
    Return channel mapping with fallback support for different beta file formats.
    Tries multiple column name variations to support both standard and Kalman formats.
    """
    base_mapping = {
        'google campaigns': ['Beta_Google_Impression'],
        "traffic(all sku's)": [
    ...
    # For each channel, find which column actually exists in the beta file
    final_mapping = {}
    for channel, possible_cols in base_mapping.items():
        found_col = None
        for col in possible_cols:
            ...
            if col in beta_df.columns:
                found_col = col
                break
        final_mapping[channel] = found_col
