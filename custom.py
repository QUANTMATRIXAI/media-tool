...
def run_model_for_product(df, product_name, target_var, all_vars, mandatory_vars, 
    ...
    try:
        ...
        remaining_vars = [v for v in all_vars if v not in mandatory_vars]
        num_to_select = max_vars - len(mandatory_vars)
...
def main():
    ...
    with st.sidebar:
        ...
        non_negative_constraints = set()
        non_positive_constraints = set()
        ...
        if uploaded_file is not None:
            ...
            if 'Product title' in df.columns:
                ...
                run_all_products = st.checkbox("🔄 Run for All Products", value=False,
                                              help="Run model for all products and show summary")
                
                if not run_all_products:
            ...
            max_vars = st.slider("Max Variables", 5, 20, 10)
            
            if run_all_products:
                r2_threshold = st.slider("R² Threshold (%)", 0, 100, 70, 5,
                                        help="Show products with R² above this threshold") / 100
            else:
                r2_threshold = 0.70
            ...
            # Date selector for dual model (only for single product)
            partial_end_date = None
            if run_dual_model and not run_all_products and 'Date' in df.columns:
                st.write("📅 **Partial Model Date Range**")
                
                # Get date range for selected product
                product_data = df[df['Product title'] == selected_product].copy()
                if len(product_data) > 0 and 'Date' in product_data.columns:
            ...
            # Mandatory variables
            st.write("🔒 Mandatory Variables")
            use_google_trends = st.checkbox('google_trends', value=True)
            use_price = st.checkbox("Product variant price", value=True)
            use_discount = st.checkbox('Category Discount', value=True)
            ...
            st.subheader("🧭 Coefficient Sign Constraints")
            ...
            non_negative_selection = st.multiselect(
                "Force coefficients >= 0",
                options=available_constraint_vars,
                help="Selected predictors will start (and stay) non-negative in the ridge warm start.",
                key="non_negative_constraints"
            )
            non_positive_selection = st.multiselect(
                "Force coefficients <= 0",
                options=available_constraint_vars,
                help="Selected predictors will start (and stay) non-positive in the ridge warm start.",
                key="non_positive_constraints"
            )
            non_negative_constraints = set(non_negative_selection)
            non_positive_constraints = set(non_positive_selection)
            overlap_constraints = non_negative_constraints & non_positive_constraints
            if overlap_constraints:
                st.warning(f"Constraints overlap for: {', '.join(sorted(overlap_constraints))}. "
                           "Removing them from the non-positive list.")
            ...
            use_log = False
            standardize = st.checkbox("Standardize Predictors", value=True)
        ...
        if run_all_products and st.session_state.all_products_results is not None:
            ...
            if len(high_r2_results) > 0:
                ...
                tab1, tab2, tab3, tab4, tab5 = st.tabs([
                    "📈 Summary", "📉 Predictions", "🎯 Scatter", "📊 Coefficients", "💾 Download"
                ])
                ...
                with tab5:
                    st.subheader("💾 Download Product Results")
            ...
            tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs([
                ...
                "🔄 Q & R Evolution", "🔍 Selection Process", "💾 Download", "🧪 Validation Tests"
            ])
            ...
            with col1:
                st.metric("Negative Predictions", (y_pred < 0).sum(), 
                         delta="✓ None" if (y_pred < 0).sum() == 0 else "✗ Found",
                         delta_color="normal" if (y_pred < 0).sum() == 0 else "inverse")
            
            with col2:
                extreme = (y_pred > np.max(y)*3).sum()
                st.metric("Extreme Predictions (>3x max)", extreme,
                         delta="✓ None" if extreme == 0 else "⚠ Found",
                         delta_color="normal" if extreme == 0 else "inverse")
